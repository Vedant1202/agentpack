...
### `agentpack validate`
Validates the structural integrity of a context pack, ensuring all expected files and tables exist and are properly referenced in the manifest. When a `map.yml` is present, its referential integrity is checked too — every node's `chunk_ids` and `source_id` must resolve against the manifest (an absent map is not an error).
