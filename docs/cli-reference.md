...
- `--no-graph`: Skip building the corpus concept graph (`graph.yml`). The graph is generated by default.
...
min_docs             = 2      # >= 1    distinct documents required to promote a concept
...
```bash
agentpack graph <pack_dir>
agentpack graph <pack_dir> --with-similarity
```
- `--with-similarity`: Also (re)compute `similar_to` edges from an already-built vector index. Requires `agentpack index` to have run; without a vector index the step is skipped with a warning rather than failing.
- `--quiet`: Suppress progress output.

The rebuild reads `manifest.yml` and `map.yml` off disk — the same code path `pack` uses — so its output is byte-identical to the pack-time graph apart from the `generated_at` timestamp. Gate values are reused from the existing `graph.yml`'s recorded `params:` block rather than re-read from `agentpack.toml`, which is what makes a rebuild reproducible.
