...
- [x] Packed `demo_corpus/` (T8 evidence above — 0 concepts, 0 references, 4 singleton communities; verified as a correct zero, not a bug, back at T2/T3).
- [x] Packed `benchmarks/financebench_sample`. **Full 42-document corpus OOM-killed** (exit 137) on this machine (8GB RAM) — parsing/chunking completed cleanly for all 42 sources (9615 chunks, valid manifest.yml), but the process died during map/graph construction. Not a code bug in T1–T8 (chunking is the do-not-touch pipeline; the graph build itself never got far enough to run). Substituted a 21-document subset (AMCOR ×5, BESTBUY ×4, GENERALMILLS ×3, CORNING ×3, COCACOLA ×3, 3M ×3 — chosen for real same-company overlap across filing years) which packed cleanly and validated clean.
- [x] **Top concepts (19 promoted total):** of the 19, 2 are genuine named business segments (`Amcor Flexibles North` — truncated at the source YAKE extraction, should read "...North America"; `Corning Display Technologies`); the rest split between generic SEC-filing boilerplate (`Company Annual Report`, `Financial Statements`, `UNITED STATES`, `Chief Financial Officer`, the company's own letterhead name `CORNING`/`CORNING INCORPORATED`) and near-duplicate YAKE fragments of the same underlying date-boilerplate idea that the slug recipe doesn't merge (`FISCAL YEAR` / `Fiscal Year Fiscal` / `Year Fiscal Year`; `year ended December` / `years ended December`; `year ended June` / `Years ended June`), plus one raw table-column-header artifact (`Domestic International Total`) and one date-stamp fusion (`Coca-Cola Company February`).
- [x] **Communities (9 total):** coherent, not scrambled — each lands cleanly on either a real segment cluster (`Amcor Flexibles North`, `Corning Display Technologies`), a shared-boilerplate cluster (`year ended December`, `Financial Statements`, `FISCAL YEAR`), or a legitimate singleton for a structurally different filing (the lone 8-K, an isolated 10-Q).
- [x] **Isolated documents:** `3M_2022_10K.pdf`, `AMCOR_2022_8K_dated-2022-07-01.pdf`, `BESTBUY_2024Q2_10Q.pdf`. The AMCOR one is an 8-K (a short, event-driven disclosure, structurally unlike the annual/quarterly reports it was packed alongside) — its isolation is expected, not a red flag.
- [x] Human review: **are promoted concepts real topics or noise?** Mostly noise on this corpus type — ~2 of 19 are real segment names, the rest are SEC-filing boilerplate or YAKE near-duplicate fragments. **Are communities coherent?** Yes. **Is `df_cap = 0.30` sane on real filings?** It's doing real filtering work (the top concept sits at 6/21 ≈ 29%, just under the cap — not a no-op), but it filters on raw cross-document frequency, which can't distinguish "common because it's a real recurring company topic" from "common because every SEC filer's boilerplate says it." A generic corpus-agnostic frequency cap structurally favors regulatory boilerplate over distinctive content on this corpus type.
- [x] **Do not begin Phase B without an explicit go.** Findings above reported to the user; Phase B not started.

---

## Phase B — Similarity edges ⛔ *gated by checkpoint*

### B1 · `similar_to` edges at index time ✅ DONE
- [x] `grapher.py`: `add_similarity_edges(pack_dir, params=None)` — section centroid = mean of the section's own `chunk_ids`' already-normalized vectors (read from `indexes/vector_index.npy` + `vector_meta.json`, chunk_id → row index), **re-normalized** (verified live: the mean of unit vectors isn't itself unit length, so a plain dot product of two raw means wouldn't be a true cosine similarity). Cosine ≥ `params["similarity_threshold"]`, section↔section edges across **different documents only**, `basis: embedding`. Params resolution mirrors the `agentpack graph` rebuild command exactly: an explicit `params` argument wins, otherwise reused verbatim from whatever's already recorded in the target graph.yml's own `params:` block, never read from ambient `agentpack.toml`.
- [x] Merge + idempotency: this function owns the entire `similar_to` relation outright — every run strips ALL prior `similar_to` edges from graph.yml and replaces them with the freshly computed set, rather than a per-edge keyed upsert. Chosen deliberately over upsert: a wholesale replace stays correct even if the underlying embeddings changed between runs (an upsert can't detect and remove an edge that's no longer valid), and it still satisfies the todo's own idempotency requirement (re-running with unchanged inputs reproduces byte-identical output). Communities recomputed via the existing `_compute_communities` after the merge, on the full node/edge set including the new edges.
- [x] Wiring, both permitted call sites: (1) a 7-line **append-only** hook at the end of `build_vector_index` (retrieve.py) — `git diff` shows pure addition, nothing removed or modified elsewhere in the file (pasted below); relies on `add_similarity_edges`'s own never-raise posture rather than a redundant second try/except, mirroring how `write_graph`'s call site in `pack.py` already trusts `write_graph`'s own internal safety. (2) `agentpack graph --with-similarity` (cli.py) for explicit reruns — calls `add_similarity_edges` with no explicit params so it reads back exactly what `write_graph` just recorded moments earlier in the same command.
- [x] Never builds `graph.yml` or the vector index itself — both must already exist; absent either, degrades to a stderr note + `False` return, never a crash, staying a pure post-processor per spec §3 (mirrors `build_graph`'s own posture from T1).
- [x] Tests (`tests/test_grapher.py`, 10 new; `tests/test_graph_cli.py`, 2 new — 12 total): edge created at cosine=1.0 (default 0.80 threshold); no edge at cosine=0.0; threshold honored (cosine=0.6 rejected at 0.80, admitted at a lowered 0.5 — exact values verified live: `dot([1,0],[0.6,0.8]) == 0.6`); same-document sections excluded even at cosine=1.0; idempotent rerun produces byte-identical output (modulo `generated_at`); no-op with a warning when no `graph.yml` exists; no-op with a warning when no vector index exists; a `graph.yml` with zero `similar_to` edges (pack-time only) still passes `validate_pack`; one real end-to-end test through the actual `build_vector_index` (mocked embeddings, real `write_pack`) proving the retrieve.py hook itself fires, not just the isolated function; two CLI tests for `--with-similarity` (refreshes edges; the flag's *absence* means a plain rebuild does **not** preserve a previously-merged `similar_to` edge — `write_graph` always rebuilds the structural graph from scratch with no knowledge of any prior merge, a real and worth-flagging interaction, not a bug: similarity is explicitly a separate, opt-in layer per spec, not baked into the base rebuild).
- [x] **Two test-design corrections caught before they became false signal, not bugs in the implementation:** (1) an initial same-document-exclusion fixture used only 1 total document, which the pre-existing `_MIN_SUCCESSFUL_SOURCES=2` gate (T1) correctly refuses to build a graph for at all — fixed by adding a second, unrelated filler document. (2) An initial "recompute communities after merge" test asserted that bridging two disconnected 2-node pairs with one similarity edge would merge them into the same Louvain community — verified live (networkx 3.6.1, seed=42) that Louvain instead keeps them in **separate** communities even after the bridge, a real, well-known modularity resolution-limit effect on small graphs, not a defect in `_compute_communities`. Asserting a specific merge outcome would have been testing Louvain's optimization choice, not this code, so the test was redesigned to prove recomputation happens (by planting an obviously-stale `communities:` block before merging and confirming it gets overwritten with a fresh, internally-consistent one) rather than predicting a specific clustering result.

**Acceptance:** ✅ idempotency proven (dedicated test); `retrieve.py` diff is append-only (below); full suite = baseline + 12.
**Verify:** ✅ RED confirmed first (9 of 10 new `test_grapher.py` tests failed on `ImportError: add_similarity_edges`; the 10th — validate-still-passes — passed vacuously and was re-checked post-implementation). `PYTHONPATH="$PWD/src" ./venv/bin/python -m pytest tests/test_grapher.py -v -k similarity` → **10 passed**; `tests/test_graph_cli.py -k similarity` → **2 passed**. Existing `tests/test_retrieve.py` (17 tests, several with a mocked `build_vector_index` and no `graph.yml` present) re-verified green unchanged — confirms the new hook's no-op path is exercised for free by pre-existing tests, not just new ones. Full suite: `PYTHONPATH="$PWD/src" ./venv/bin/python -m pytest tests/ -q` → **285 passed, 1 pre-existing failure** (`test_run_eval`) — exactly 273 (T8 baseline) + 12 new, zero regressions.

**`retrieve.py` diff (genuinely append-only):**
```diff
...
- **Real-corpus check, and an honest, worth-recording finding:** ran `agentpack index` (real fastembed, no mocking) on the same 21-document financebench subset from the checkpoint. Produced **210 `similar_to` edges — literally every possible cross-document pair** (C(21,2) = 210). Not taken at face value: computed the actual pairwise cosine distribution directly. Min **0.844**, max 0.998, median 0.962 — every single pair clears the 0.80 default threshold. Root cause, confirmed by inspection: these are all PDFs, and PDFs get **zero granular section extraction** in this pipeline (a pre-existing `mapper.py` property, not something B1 touches) — every document collapses to one `(root)` section holding all of its chunks (up to 231 in this corpus). A section centroid over 200+ chunks spanning an entire financial filing averages out to a generic "this is a dense SEC filing" vector, and those cluster tightly together in embedding space regardless of which specific company or topic is discussed. The **mechanism is correct and thoroughly tested** (threshold honored precisely, cross-doc-only honored, idempotent, communities recomputed) — the **signal is close to non-discriminative on this corpus shape specifically**, for a diagnosed, structural reason (whole-document pooling from missing section granularity), mirroring the T2 concept-quality finding from the checkpoint in spirit if not in cause. Per the user's explicit instruction ("go ahead with Phase B, tune concepts later"), this is left as a known tuning target rather than addressed now.
**Commit:** `9080160`.
